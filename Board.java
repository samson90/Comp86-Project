...
public class Board extends JComponent implements MouseListener{
  ...
  private Item selectItem; //player can select an item in his current room	                           //by clicking on it.
  private int select_x, select_y;//coordinates where last mouse click was.
  private Game game;
  private Main main;
  ...
  private boolean draw;//boolean that makes the board visible.
  ...
  //Takes an array of rooms and doors and initalizes the layout and doors arrays.
  ...
  public void mousePressed (MouseEvent event){
    ...
    //Enables certain buttons in the UI.
  ...
  public void paintComponent(Graphics g){
    ...
    //checks for game over and posts game over screen
    if(game.getGameOver()){
      g.setColor(Color.black);
      g.fillRect(0, 0, width, height);
      g.setColor(Color.white);
      g.drawString("Congratulations! You saved the Station!", 450, 100);
      g.drawString("Sub Total: " + game.getScore(), 500, 200);
      g.drawString("Gold Bonus: " + game.getGold() + " x 5 = " + 
                   game.getGold() * 5, 500, 250);
      g.drawString("Time Bonus: " + (3600 - game.getTime()) / 100, 500, 
                   300);
      g.drawString("Total Score: " + (game.getScore() + game.getGold() * 5
                   + (3600 - game.getTime()) / 100), 500, 350);
    }
    //checks whether player dies and posts game over screen
    if(game.getDeath()){
      g.setColor(Color.black);
      g.fillRect(0, 0, width, height);
      g.setColor(Color.white);
